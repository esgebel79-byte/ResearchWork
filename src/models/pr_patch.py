...
class PRPatchModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor, ews: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        # MultiheadAttention expects (batch, seq, dim) when batch_first=True
        attn_out, _ = self.cross_attn(query=comp_tokens, key=ews_tokens, value=ews_tokens)
        # attn_out: (b, Lq, d_model) -> pool across tokens to get global
...
class PhysicsRegularizedLoss(nn.Module):
    ...
    forward(preds, targets, inputs=None, ews=None)
    где preds/targets: (batch, C) прогноз и истинные значения на следующий шаг,
    inputs: (batch, seq_len, C) история для вычисления S[t], I[t], R[t]
    ews: (batch, seq_len) или (batch, seq_len, n_ews) — используется для lambda_t
    """
    ...
    def __init__(self, lambda_base: float = 0.1, alpha: float = 1.0, device: Optional[torch.device] = None):
        ...
        # raw params then softplus to ensure positivity
    ...
    def forward(self, preds: torch.Tensor, targets: torch.Tensor,
        ...
        preds, targets: (batch, C) next-step
        inputs: (batch, seq_len, C) history including S[t], I[t], R[t]
        ews: (batch, seq_len) or (batch, seq_len, n_ews)
        """
        device = preds.device
        ...
        # If inputs have at least 3 channels, attempt SIR-style residuals using first 3 channels
        if inputs.shape[2] >= 3:
            S_hist = inputs[:, :, 0]
            I_hist = inputs[:, :, 1]
            R_hist = inputs[:, :, 2]
            ...
            # Generic fallback: penalize large second-differences (acceleration) across available channels
            # compute per-sample mean absolute second difference over last 3 timesteps
            if inputs.shape[1] >= 3:
        ...
        # compute lambda_t using ews: aggregate last-window EWS score if provided
        if ews is not None:
            # if ews has shape (b, seq_len, n_ews) or (b, seq_len)
            if ews.dim() == 3:
                # simple score: mean normalized across features
                score = ews.mean(dim=1).mean(dim=1)  # (b,)
            else:
                score = ews.mean(dim=1)  # (b,)
            # normalize score (z-score per batch)
        ...
        # final physics loss weighted per sample
...
def cumulative_pce(pred_seq: torch.Tensor, beta: torch.Tensor, gamma: torch.Tensor, N: torch.Tensor) -> torch.Tensor:
    """Вычислить kumулятивную Physics Consistency Error (PCE) по всей траектории.

    Предполагается, что `pred_seq` имеет форму (batch, H, 3) для компонентов [S,I,R].
    `beta` и `gamma` — скаляры (torch.Tensor или float).
    `N` — либо скаляр либо тензор формы (batch,) или (batch, H).

    Возвращает скаляр: среднеквадратичная невязка по всем временным шагам и батчам.
    """
    # convert to tensors
    ...
    # if 3 components present treat as SIR, else fallback to second-differences across channels
    if pred_seq.shape[2] >= 3:
        ...
        use_sir = False
        # compute second differences based pce
    ...
    # compute residuals for t -> t+1 over horizon H-1
    if use_sir:
        if S.shape[1] < 2:
        ...
        # if N_t is (batch,), expand to (batch, H-1)
        if N_t.dim() == 1:
        ...
        # fallback: compute mse of second differences across channels and time
...
def _synthetic_integration_test():
    """Статическая проверка: прогон через forward + backward на синтетических данных.

    Генерирует случайный батч S,I,R и EWS, прогоняет их через модель и считает лосс.
    Проверяет отсутствие ошибок размерностей и наличие градиентов для beta/gamma.
    """
    ...
    # synthetic positive S,I,R sequences
    # ensure S+I+R constant-ish
    ...
    X = torch.stack([S, I, R], dim=2)  # (batch, seq_len, 3)
    # synthetic EWS (var, ar1)
    ...
    model.train()
    preds = model(X, EWS)
    # create synthetic targets as next-step true values (shifted)
    y_true = torch.stack([S[:, -1] + (S[:, -1] * 0.0), I[:, -1] + (I[:, -1] * 0.0), R[:, -1] + (R[:, -1] * 0.0)], dim=1)
    total, data_l, phys_l = loss_fn(preds, y_true, inputs=X, ews=EWS)
    print("Loss components:", float(data_l), float(phys_l), float(total))
    total.backward()
    # check gradients for beta/gamma
